...
export async function computeExpressionsOverlay(date, broadcastings, expByRes) {
  ...
  newRenderer = function(){
    ...
    ns.render = function(g, settings) {
      ...
      // Draw OVERLAY
      bgImage = ns.drawLayerOnTop(bgImage,
        ns.drawOverlayLayer(ns.settings, broadcastings, expByRes)
      )
    ...
    ns.drawOverlayLayer = function(options, broadcastings, expByRes) {
      ns.log("Draw overlay...")
      
      options = options || {}
      options.overlay_grid_size = 120 // In px
      options.overlay_label_alternating_offset = 40 // In px
      options.overlay_minimum_exp_count = 10
      /*
      ...
      */
      ...
      for (let xGrid=0; xGrid<dim.w+options.overlay_grid_size; xGrid += options.overlay_grid_size) {
        for (let yGrid=0; yGrid<dim.h+options.overlay_grid_size; yGrid += options.overlay_grid_size) {
          ...
          if (Object.keys(nodeIndex).length >= options.overlay_minimum_exp_count) {
            // Extract hashtags
            let expCounts = {}
            broadcastings.forEach(b => {
              if (nodeIndex[b.broadcaster_id]) {
                let expressions = (expByRes[b.resource_id] || [])
            ...
            if (mainExpCount >= options.overlay_minimum_exp_count) {

              let exp = mainExp
              exp = ns.truncateWithEllipsis(exp, 24)
              
              let yOffset = (xBeat-1) * options.overlay_label_alternating_offset
              drawText(ctx, exp, xGrid, yGrid+12 + yOffset, "center", "#FFFFFF", 0, "bold 32px Raleway")
            }
          }
        }
        xBeat = (xBeat+1)%3
