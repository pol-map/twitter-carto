...
import { computeExpressionsOverlay } from "./viz_expressions.js";
import * as StackBlur from "stackblur-canvas";

let settings = {}
settings.sdate = "2022-07-22"
settings.edate = "2022-10-21"
...
async function encodeFrame() {
    ...
    encoder.delete();
    fs.writeFileSync(`data/Main expressions from ${settings.sdate} to ${settings.edate}.mp4`, Buffer.from(uint8Array));
    console.log("Done.")
  }
}
...
async function getOverlay(date, folder) {
  // Load broadcastings
	let broadcastings = []
  let filePathB = `${folder}/broadcastings.csv`
  if (fs.existsSync(filePathB)) {
    try {
      // Load file as string
      let csvString = fs.readFileSync(filePathB, "utf8")
      // Parse string
  ...
  // Load resources_7days_aggregated_expressions
	let resources = []
  let filePathR = `${folder}/resources_7days_aggregated_expressions.csv`
  if (fs.existsSync(filePathR)) {
    try {
      // Load file as string
      let csvString = fs.readFileSync(filePathR, "utf8")
      // Parse string
      resources = d3.csvParse(csvString);
      console.log(`resources loaded (${resources.length} rows)`)

    } catch (error) {
      console.error(`An error occurred during the loading and parsing of resources`, error)
    }
  } else {
    console.warn(`resources not found`)
  }

  // Build index
  let expByRes = {}
  resources.forEach(res => {
  	expByRes[res.id] = JSON.parse(res.covering_exp)
  })

  // Get heatmap
  const oImgd = await computeExpressionsOverlay(date, broadcastings, expByRes)
  return oImgd
}
